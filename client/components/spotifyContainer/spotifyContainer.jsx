...
function SpotifyContainer({
	...
	const onPlaylistClick = () => {
		...
		const songURIArray = [];
		const filteredSpotifyTrackList = spotifyTrackList.filter(
			(track) => track.include
		);
		...
		const filteredURIArray = songURIArray.filter(
			(el) => el !== 'spotify:track:null'
		);
	...
	return (
		<div className={styles.playlistTitle}>
			...
			{!isLoggedIn && (
				<p>
					<span>Love this Playlist?</span> Link your Spotify account and save
					this playlist so you can listen anytime!
				</p>
			)}
			...
			{!isLoggedIn ? (
				...
				<div className={styles.playlistAndLogout}>
					...
					<button
						type="button"
						className={styles.spotifyLogoutButton}
